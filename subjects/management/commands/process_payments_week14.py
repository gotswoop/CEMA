...
class Command(BaseCommand):
	...
	help = 'processes payments for week 14 survey (Mon, Tue, Wed)'

	def handle(self, *args, **kwargs):

		date_start = datetime.now().replace(hour=00, minute=00, second=0, microsecond=0) # Settings datetime time to start of day
		date_now = datetime.now() # 
	
		users = {}		
		subjects = Subjects.objects.filter(test_account=0, deleted=0, optout=0).order_by('study_id')
		for su in subjects:
			try:
				# Check if this user finished (status =2), week 14(survey_number=14) today (start_datetime > today and now)
				week14 = SurveyLinks.objects.filter(survey_number=14, 
													status=2, 
													study_id=su, 
													start_datetime__gt=date_start, 
													start_datetime__lte=date_now
												).get()
			except SurveyLinks.DoesNotExist:
				# User hasn't completed week 14 survey. Move to next user.
				continue
			
			try:
				# select * from schedule_plus where survey_link = 211;
				most_recent_scheduled_survey = SchedulePlus.objects.get(study_id=su, survey_link=week14)
			except SchedulePlus.DoesNotExist:
				print("SHOULD NOT BE IN HERE!!!!")
				print(su.pk, su.fullname(), su.cohort, week14.pk, " - Nothing Scheduled")
				continue
			
			week_n_day = most_recent_scheduled_survey.survey
		...
		out_file = 'Week14_Payments_for_' + datetime.today().strftime('%Y.%m.%d') + '.xls'
