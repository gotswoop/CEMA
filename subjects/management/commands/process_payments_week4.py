...
class Command(BaseCommand):
	...
	def handle(self, *args, **kwargs):
		...
		users = {}		
		subjects = Subjects.objects.filter(test_account=0, deleted=0, optout=0, cohort=2).order_by('study_id')
		for su in subjects:
			try:
			...
			date_pay_5_weeks = survey_obj.start_datetime + timedelta(weeks=5)
			date_pay_10_weeks = survey_obj.start_datetime + timedelta(weeks=10)
		...
		for key, value in users.items():
			...
			if row[20]:
				PaymentSummary.objects.create(
					...
					payment_date = date_pay_today.replace(hour=0, minute=0, second=0),
			...
			if row[21]:
				PaymentSummary.objects.create(
					...
					payment_date = date_pay_5_weeks.replace(hour=0, minute=0, second=0),
					payment_message = '-',
				)
			...
			if row[22]:
				PaymentSummary.objects.create(
					...
					payment_date = date_pay_10_weeks.replace(hour=0, minute=0, second=0),
					payment_message = '-',
				)
